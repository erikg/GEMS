  ...
  while (syn[x] != NULL)
    {
      ...
      n =
	gtk_ctree_insert_node (GTK_CTREE (ctree1), NULL, NULL, text, 5, NULL,
...
on_toolbar_prev_clicked (GtkButton * button, gpointer user_data)
{
  ...
  if (gtk_ctree_node_is_visible (GTK_CTREE (ctree), n) != GTK_VISIBILITY_FULL)
  ...
  m =
    ((mboxs *)
     gtk_object_get_user_data (GTK_OBJECT
			       (lookup_widget
				(GTK_WIDGET (button), "delete_mbox"))))->name;
  db_dropmbox (m);
...
face_gnome_o_rule_setpeice (GtkWidget * w, gpointer * trash)
{
  ...
  GtkMenuItem *mi;
  r =
    (rule *) gtk_clist_get_row_data (GTK_CLIST (lookup_widget (w, "clist1")),
				     rule_row_current);
  free (r->peice);
...
on_toolbar_rule_clicked (GtkButton * button, gpointer user_data)
{
  ...
  gtk_combo_set_popdown_strings (GTK_COMBO (lookup_widget (r, "combo2")), gl);
  gtk_signal_connect (GTK_OBJECT
		      (gtk_option_menu_get_menu
		       (GTK_OPTION_MENU (lookup_widget (r, "optionmenu2")))),
...
on_rule_mbox_changed (GtkEditable * editable, gpointer user_data)
{
  ...
  r->mbox =
    g_strdup (gtk_entry_get_text
	      (GTK_ENTRY
	       (GTK_COMBO (lookup_widget (GTK_WIDGET (editable), "combo2"))->
		entry)));
}
